...
{
	//EmulationLoop() ;
	//SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION, "Test", "Inside RenderGame", NULL);
 	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
 	glLoadIdentity();
 	glRasterPos2i(-1, 1);
	glPixelZoom(1, -1);
 	glDrawPixels(160, 144, GL_RGB, GL_UNSIGNED_BYTE, m_Emulator->m_ScreenData);
	//LogMessage::GetSingleton()->DoLogMessage("Rendergame"+ m_Emulator->m_ScreenData,true);
...
{
	//SDL_ShowSimpleMessageBox(SDL_MESSAGEBOX_INFORMATION,"Test","Inside Create Window",NULL);
	if( SDL_Init( SDL_INIT_EVERYTHING ) < 0 )
	{
		return false ;
	}
	sdlWindow = SDL_CreateWindow("GB", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, windowWidth, windowHeight, SDL_WINDOW_OPENGL);
	if( sdlWindow == NULL)
	{
		return false ;
	}
